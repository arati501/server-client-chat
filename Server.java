...
public class Server1 {
	 ...
	 */
	static int sendMessageOfTheDay(DataOutputStream outputStream, String[] message, int currentMessage,
			int messageCount, Socket client) {
		try {
	 ...
	 * users in the file usersDir.txt and will print an error if they are already
	 * logged in
	 */
	static String loginUser(DataOutputStream outputStream, String activeUser, String loginArgs, Socket clientSocket) {
		try {
			...
			if (!activeUser.equals("")) {
				...
				try {
					inputScanner = new Scanner(new File("usersDir.txt")); // this will check the usernames stored in the
																			// server
					...
					if (inputLoginPassword.length != 3) {
						...
						while (inputScanner.hasNextLine()) {
							...
							if ((inputLoginPassword[0].equals(loginPasswordRecord[0]))
									&& (inputLoginPassword[1].equals(loginPasswordRecord[1]))) {
								int i = 0;
	...
	/*
	 * To logout user from the server. Throws error if trying to logout without
	 * login
	 */
	...
	static String saveUserMessage(DataInputStream dataInputStream, DataOutputStream dataOutputStream,
			String[] messageOfTheDay, int messageCount, String message, String currentUser, Socket client) {
		try {
			dataOutputStream = new DataOutputStream(client.getOutputStream());
			if (currentUser.equals("")) {
	...
	/*
	 * Shutdown server allowed only for root user. Throws error if non root user
	 * tries to shutdown
	 */
	...
	static class clientHandler {
		...
		public void Socketrun() {
			try {
				...
				while ((message = dataInputStream.readUTF()) != null && proceed == true) {
					...
					if (message.startsWith("LOGIN", 0)) {
						...
						if (!inputValidation.contains(message)) {
							System.out.println("INVALID COMMAND PROVIDED"); // validating if any invalid command are
																			// provided
						}
						switch (message) {
						case "MSGGET":
							activeMessageCount = sendMessageOfTheDay(dataOutputStream, messageOfTheDay,
									activeMessageCount, messageCount, client);
							break;
						case "LOGOUT":
							activeUser = userLogout(dataOutputStream, activeUser, client);
							break;
						case "MSGSTORE":
							String msg = saveUserMessage(dataInputStream, dataOutputStream, messageOfTheDay,
									messageCount, message, activeUser, client); // the next empty element in array is
				...
				// close input and output stream and socket. we only reach this if the user
				// calls quit() successfully
				client.close();
			} catch (IOException ex) { // handling disconnections here ensures that we can clean up the user's info
										// even if the disconnection was abrupt
				System.out.println("User disconnected.");

				if (socket == client) {
					if (!activeUser.equals("")) {
						activeUser = userLogout(dataOutputStream, activeUser, client);// user gets logged out if they
																						// are logged in
					}
