...
public class Client1 {
	...
	public static void main(String[] args) {
		...
		List<String> inputValidation = Arrays.asList("MSGGET", "MSGSTORE", "QUIT", "SHUTDOWN", "LOGOUT", "WHO");
		...
		if (clientSocket != null && dataOpStr != null && dataIpStr != null) {
			try {
				while (keepRunning == true) {
					userInput = input.readLine();
					if (!inputValidation.contains(userInput) && !(userInput.startsWith("LOGIN", 0)) && !(userInput.startsWith("SEND", 0))) {
	...
	/*
	 * messageHandler(): This method handles incoming server messages and processes
	 * them.
	 */
	static class messageHandler implements Runnable {
		...
		public void run() {
			...
			try {
				...
				if (result == 1) {
					System.out.println(
							"The server is at maximum capacity and cannot accept additional connections at this time. \n");
				} else {
					while (true) {
						if (!is.ready()) {
							try {
							...
							if ((message.equals("Thank you for using the server."))
									|| (message.equals("Server shutting down...")))
								System.exit(0);
						}
